...
const TemplateDeskripsiPengetahuan = ({ dataTemplatePengetahuan }) => {
  const { user } = useUser();
  const initialValue = {
    id: "",
    judul: "",
  ...
  const handleSubmit = async () => {
    ...
    if (data) {
      const index = dataTemplatePengetahuan?.findIndex(
        (item) => item.id == formData.id
      );
      dataTemplatePengetahuan[index].prolog = formData.prolog;
      dataTemplatePengetahuan[index].epilog = formData.epilog;
  ...
  const template = dataTemplatePengetahuan?.map((item) => {
    return {
      id: item.id,
      ...
      <div className="card-header p-4 card-header-ss">
        <div className="d-flex justify-content-between align-items-center">
          <h4 className="fw-extrabold color-dark mb-sm-0 mb-3">
            Daftar Template Deskripsi Pengetahuan{" "}
          </h4>
          {/* {user?.role == "guru" && (
            <button
              type="button"
...
export default TemplateDeskripsiPengetahuan;
